...
let%test "codim=2 independent, x₁=1/2" =
  let point = Point.from_list [ Q.( // ) 1 2 ] in
  let result = Simple_system_solver.solver 3 2 system_2_indep point in
  let e1 = Cle_vec.expr result 1 and e2 = Cle_vec.expr result 2 in
  Lin_expr.equal e1 (Lin_expr.from_list [ Q.( // ) 1 2; Q.zero ])
  && Lin_expr.equal e2 (Lin_expr.const (Q.( // ) 1 2))
  && constraints_ok result point

let%test "codim=2 independent, x₁=1" =
...
(* ---- codim=2, coupled ----
   ...
   Solve order: x₃ first, then x₂
   Expected: x₂ = 4/15 x₁ + 1/15, x₃ = 1/15 x₁ + 4/15
   At x₁=1/2: x₂ = 1/5, x₃ = 3/10 *)

let%test "codim=2 coupled" =
  let t1 =
    Mu_term.(
      strong_disj
        (scalar_mult (Q.( // ) 1 4) (var 1))
  ...
  let point = Point.from_list [ Q.( // ) 1 2 ] in
  let result = Simple_system_solver.solver 3 2 f point in
  let e1 = Cle_vec.expr result 1 and e2 = Cle_vec.expr result 2 in
...
(* ---- codim=3, chain ----
   ...
   f₂ = 1/4 x₂ ⊕ 1/2 x₃   (body for x₃)
   f₃ = 1/4 ⊕ 1/2 x₄       (body for x₄)
   Expected: x₂ = 1/2 x₁, x₃ = 1/4 x₁, x₄ = 1/2
   At x₁=1/2: x₂ = 1/4, x₃ = 1/8, x₄ = 1/2 *)

let%test "codim=3 chain" =
  let t1 =
    Mu_term.(
      strong_disj
        (scalar_mult (Q.( // ) 1 4) (var 1))
  ...
  let point = Point.from_list [ Q.( // ) 1 2 ] in
  let result = Simple_system_solver.solver 4 3 f point in
...
(* ---- codim=4, chain ----
   ...
   f₃ = 1/4 x₃ ⊕ 1/2 x₄   (body for x₄)
   f₄ = 1/4 ⊕ 1/2 x₅       (body for x₅)
   Expected: x₂ = 1/2 x₁, x₃ = 1/4 x₁, x₄ = 1/8 x₁, x₅ = 1/2
   At x₁=1/2: x₂ = 1/4, x₃ = 1/8, x₄ = 1/16, x₅ = 1/2 *)

let%test "codim=4 chain" =
  let t1 =
    Mu_term.(
      strong_disj
        (scalar_mult (Q.( // ) 1 4) (var 1))
  ...
  let point = Point.from_list [ Q.( // ) 1 2 ] in
  let result = Simple_system_solver.solver 5 4 f point in
  let e1 = Cle_vec.expr result 1
  and e2 = Cle_vec.expr result 2
...
(* ---- codim=2, m=2 (two free variables) ----
   f₁ = (1/4 x₁ ⊕ 1/4 x₂) ⊕ 1/2 x₃   (body for x₃)
   f₂ = 1/4 ⊕ 1/2 x₄                    (body for x₄)
   Expected: x₃ = 1/2 x₁ + 1/2 x₂, x₄ = 1/2
   At (x₁,x₂)=(1/2,1/4): x₃ = 3/8, x₄ = 1/2 *)

let%test "codim=2 m=2" =
  let t1 =
    Mu_term.(
      strong_disj
        (strong_disj
  ...
  let result = Simple_system_solver.solver 4 2 f point in
  let e1 = Cle_vec.expr result 1 and e2 = Cle_vec.expr result 2 in
